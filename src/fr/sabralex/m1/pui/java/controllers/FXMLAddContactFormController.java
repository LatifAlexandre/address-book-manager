...
public class FXMLAddContactFormController implements Initializable {
	...
	ObservableList<String> numberTypes = FXCollections.observableArrayList("Mobile", "Office", "Home", "Fax", "Pager");
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		typeCol.setCellValueFactory(
				(TreeTableColumn.CellDataFeatures<NumberObject, String> param) -> param.getValue().getValue().type);

		JFXTreeTableColumn<NumberObject, String> numberCol = new JFXTreeTableColumn<>("Number");
		numberCol.setPrefWidth(150);
		numberCol.setCellValueFactory(
				(TreeTableColumn.CellDataFeatures<NumberObject, String> param) -> param.getValue().getValue().number);
		...
		final TreeItem<NumberObject> root = new RecursiveTreeItem<NumberObject>(numbers,
